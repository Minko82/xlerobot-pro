...
def get_xbox_key_state(joystick, keymap):
    ...
    for action, control in keymap.items():
        if control == 'left_trigger':
            ...
            # Also expose analog value: normalize from -1..1 to 0..1
            state['gripper_analog'] = (axes[2] + 1) / 2 if len(axes) > 2 else 0.0
        elif control == 'right_trigger':
            state[action] = axes[5] > 0.5 if len(axes) > 5 else False
            # Also expose analog value: normalize from -1..1 to 0..1
            state['gripper_analog'] = (axes[5] + 1) / 2 if len(axes) > 5 else 0.0
        elif control == 'a':
...
def main():
    ...
    print(f"[MAIN] Controller axes: {num_axes}, buttons: {num_buttons}")
    print(f"[MAIN] Axis values at rest: {[round(joystick.get_axis(i), 3) for i in range(num_axes)]}")
    print(f"[MAIN] Button states at rest: {[joystick.get_button(i) for i in range(num_buttons)]}")
    print(f"[MAIN] NOTE: left trigger expected on axis 2, right trigger on axis 5 (resting = -1.0)")
