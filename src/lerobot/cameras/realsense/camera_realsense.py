...
class RealSenseCamera(Camera):
    ...
    def async_read(self, timeout_ms: float = 1000) -> NDArray[Any]:
        ...
        if not self.new_frame_event.wait(timeout=timeout_ms / 1000.0):
            # USB hub stalls can delay the next frame beyond timeout_ms. Return the
            # most recent buffered frame rather than crashing the recording session.
            with self.frame_lock:
                if self.latest_color_frame is not None:
                    logger.warning(
                        f"Timed out waiting for new frame from {self} after {timeout_ms} ms "
                        f"(USB stall?), returning most recent frame. "
